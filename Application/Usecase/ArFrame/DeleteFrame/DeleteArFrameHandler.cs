...
{
    public class DeleteArFrameHandler : IRequestHandler<DeleteArFrameCommand, bool>
    {
        private readonly IArFrameRepository _repo;

        public DeleteArFrameHandler(IArFrameRepository repo) => _repo = repo;

        public async Task<bool> Handle(DeleteArFrameCommand request, CancellationToken cancellationToken)
        {
